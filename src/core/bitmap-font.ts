/**
 * 5×7 monospaced bitmap font for ASCII 32-126.
 *
 * Each character is 5 pixels wide, stored as 5 rows of 1 byte each.
 * Bit 4 (0x10) = leftmost pixel, bit 0 (0x01) = rightmost pixel.
 * A set bit = black pixel (will be printed).
 *
 * Standard PC BIOS / Adafruit GFX font encoding.
 */
...
export const FONT: readonly number[][] = [
  ...
  // 0x61 'a'
  [0x00, 0x0e, 0x01, 0x0f, 0x11],
  // 0x62 'b'
  [0x10, 0x1e, 0x11, 0x11, 0x1e],
  // 0x63 'c'
  [0x00, 0x0e, 0x10, 0x10, 0x0e],
  // 0x64 'd'
  [0x01, 0x0f, 0x11, 0x11, 0x0f],
  // 0x65 'e'
  ...
  // 0x6a 'j'
  [0x02, 0x00, 0x06, 0x02, 0x12],
  // 0x6b 'k'
  [0x10, 0x12, 0x1c, 0x12, 0x12],
  // 0x6c 'l'
  ...
  [0x00, 0x0f, 0x11, 0x0f, 0x01],
  // 0x72 'r'
  [0x00, 0x16, 0x18, 0x10, 0x10],
  // 0x73 's'
  [0x00, 0x0f, 0x10, 0x0e, 0x1f],
  // 0x74 't'
...
/** Font metrics */
export const CHAR_WIDTH = 5; // pixels
export const CHAR_HEIGHT = 7; // pixels (5 glyph rows + 2 spacing rows)
export const CHAR_SPACING = 2; // pixels between characters
export const GLYPH_ROWS = 5; // actual glyph pixel rows (top 2 rows of 7 are empty)

/** Get the 5-row glyph data for an ASCII character (32-126). */
export function getGlyph(charCode: number): readonly number[] {
  if (charCode < 32 || charCode > 126) return FONT[0]; // space for unsupported
  return FONT[charCode - 32];
}
