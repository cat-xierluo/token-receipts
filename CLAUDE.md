...
**token-receipts** — NPM 包，为 AI 编程会话生成热敏打印机风格收据。支持多模型（Claude/DeepSeek/GLM/MiniMax/OpenAI/Qwen/Kimi），直接从 transcript JSONL 读取 token 用量，集成 Claude Code SessionEnd hook 自动生成。
...
编译后测试：`node bin/claude-receipts.js generate --output html`
...
- 文件：`~/.claude/projects/[project-path]/[session-id].jsonl`
